...
public enum ColorMode {
    ...
    public int makeColor(ColorMaxes colorMaxes, int a) {
        if (((a & 0xff000000) == 0) && (a <= 255)) {
            ...
            if (this == RGB) {
                int gg = (int) (PCanvas.constrain(grey, 0, maxes[0]) / maxes[0] * 255);
    ...
    public int makeColor(ColorMaxes colorMaxes, float grey) {
        ...
        if (this == RGB) {
            ...
            // Supposition : hue and alpha only
            float hh = PCanvas.constrain(grey, 0, maxes[0]) / maxes[0];
    ...
    public int makeColor(ColorMaxes colorMaxes, int a, float alpha) {
        float[] maxes = colorMaxes.get(this);
        int alph = (int) (PCanvas.constrain(alpha, 0, maxes[3]) / maxes[3] * 255);
        if (((a & 0xff000000) == 0) && (a <= 255)) {
            if (this == RGB) {
                int gg = (int) (PCanvas.constrain(a, 0, maxes[0]) / maxes[0] * 255);
                return alph << 24 | gg << 16 | gg << 8 | gg;
            } else {
                // Supposition : hue and alpha only
                float hh = PCanvas.constrain(a, 0, maxes[0]) / maxes[0];
    ...
    public int makeColor(ColorMaxes colorMaxes, float a, float b, float c) {
        float[] maxes = colorMaxes.get(this);
        if (this == RGB) {
            int aa = (int) (PCanvas.constrain(a, 0, maxes[0]) / maxes[0] * 255);
            int bb = (int) (PCanvas.constrain(b, 0, maxes[1]) / maxes[1] * 255);
            int cc = (int) (PCanvas.constrain(c, 0, maxes[2]) / maxes[2] * 255);
            return 255 << 24 | aa << 16 | bb << 8 | cc;
        } else {
            float hh = PCanvas.constrain(a, 0, maxes[0]) / maxes[0];
            float ss = PCanvas.constrain(b, 0, maxes[1]) / maxes[1];
            float bb = PCanvas.constrain(c, 0, maxes[2]) / maxes[2];
